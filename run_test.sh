...
case $CHECK_RESULT in
	...
	$EXIT_SOURCE_HAS_SETUID)
		...
		info "$(sshpass -p travis ssh -n -t -t $SSH_OPTS travis@$(< ${TRAVIS_BUILD_DIR}/docker_ip_address) "for d in \$(find /var/tmp/deb-sources -type d -name debian) ; do pushd \$d &>/dev/null && grep ^Package control 2>&/dev/null | awk -F: '{ print \$2 }' | xargs echo ; popd &>/dev/null ; done")"
